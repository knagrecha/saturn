	...
	def create_and_solve_prob(gpu_time_tuples, TASK_COUNT, makespan_opt=True, bss=None, sta=None, tga=None):
		...
		for t in range(TASK_COUNT):
			...
			for strat_idx, strat in enumerate(gpu_time_tuples[t]):
				binary_task_strategy_selection_array.append(
					LpVariable("BinarySelectedStrategy_Task_{}_Strategy_{}".format(t, strat_idx),
					           cat="Binary"))
				if bss is not None:
					binary_task_strategy_selection_array[-1].setInitialValue(bss[t][strat_idx])
			
			binary_strategy_selection_array.append(binary_task_strategy_selection_array)
			...
			for gpu_idx, gpu in enumerate(node):
				...
				for task_idx, start_time in enumerate(gpu):
					...
					for task_idx_prime, start_time_prime in enumerate(gpu):
						...
						after = before_or_after[task_idx_prime][task_idx] * M # boa = 1
						before = (1 - before_or_after[task_idx_prime][task_idx]) * M # boa = 0
		...
		if gurobi:
			solver = GUROBI_CMD(timeLimit=timeout, threads=threads, warmStart=True, options=[("intFeasTol","1e-9")])
		else:
			solver = PULP_CBC_CMD(timeLimit=timeout, threads=threads, warmStart=True, options=[("intFeasTol","1e-9")])
	...
	def solution_comparator():
		...
		if saved_makespan is None or len(interval_tga) > len(prop_tga):
			...
			for n_idx, n in enumerate(interval_sta):
				for g_idx, g in enumerate(n):
					for t_idx, t in enumerate(g):
						interval_sta[n_idx][g_idx][t_idx] = max(t - interval, 0)
...
def convert_into_comprehensible(task_list, bss, boa, tga, bna, sta):
	...
	for idx, task in enumerate(task_list):
		...
		blocked_gpus = [g_idx for g_idx, g in enumerate(blocked_node) if round(g) == 1]
		...
		for idx_prime, task_prime in enumerate(task_list):
			...
			blocked_gpus_prime = [g_idx for g_idx,
			                                g in enumerate(blocked_node_prime) if round(g) == 1]
			
			if (set(blocked_gpus).intersection(blocked_gpus_prime)):
				if round(boa[idx_prime][idx]) == 1:  # if task_prime was before task
